...
--
-- IMPORTANT:
-- This script DROPs and recreates the database for local development.
-- If you already have real data, comment out DROP DATABASE.
-- =========================================================
...
CREATE TABLE rbl_analysis_runs (
    id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    event_id BIGINT UNSIGNED NOT NULL,
    round_id BIGINT UNSIGNED NULL,
    ...
    CONSTRAINT fk_rbl_runs_event
        FOREIGN KEY (event_id) REFERENCES events(id)
        ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT fk_rbl_runs_round
        FOREIGN KEY (round_id) REFERENCES rounds(id)
        ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT fk_rbl_runs_generated_by
        FOREIGN KEY (generated_by) REFERENCES users(id)
        ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT chk_rbl_runs_status
        CHECK (status IN ('PENDING', 'COMPLETED', 'FAILED'))
) ENGINE=InnoDB;

CREATE TABLE rbl_metric_results (
    id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    ...
    CONSTRAINT fk_rbl_metric_results_run
        FOREIGN KEY (analysis_run_id) REFERENCES rbl_analysis_runs(id)
        ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT fk_rbl_metric_results_criterion
        FOREIGN KEY (criterion_id) REFERENCES scoring_criteria(id)
        ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT chk_rbl_metric_type
        CHECK (metric_type IN ('ICC', 'KRIPPENDORFF_ALPHA', 'VARIANCE', 'STD_DEV', 'MEAN'))
) ENGINE=InnoDB;
...
CREATE INDEX idx_rbl_runs_event_round ON rbl_analysis_runs(event_id, round_id);
...
-- RBL demo result
INSERT INTO rbl_analysis_runs
(id, event_id, round_id, generated_by, status, notes)
VALUES
(1, 1, 1, 3, 'COMPLETED', 'Demo RBL analysis run.');

INSERT INTO rbl_metric_results
(analysis_run_id, criterion_id, metric_type, metric_value, sample_size, notes)
VALUES
(1, 5, 'ICC', 0.820000, 2, 'Demo ICC for Technical Quality'),
(1, 6, 'KRIPPENDORFF_ALPHA', 0.760000, 2, 'Demo alpha for Innovation'),
(1, NULL, 'MEAN', 7.887500, 2, 'Demo mean weighted score');
